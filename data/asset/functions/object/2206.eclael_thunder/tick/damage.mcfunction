...
# ヒット判定
    ...
    execute if predicate api:global_vars/difficulty/min/3_blessless unless entity @s[tag=2206.Small] as @a[tag=!PlayerShouldInvulnerable,distance=..4.9] run tag @s add 2206.Hit
    execute if predicate api:global_vars/difficulty/min/3_blessless if entity @s[tag=2206.Small] as @a[tag=!PlayerShouldInvulnerable,distance=..3.5] run tag @s add 2206.Hit
