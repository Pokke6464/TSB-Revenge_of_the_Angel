#> asset:object/8001.eclael_thunder_red/init/
#
# Objectのinit時の処理
#
# @within asset:object/alias/8001/init

# Smallがtrueの場合はBlesslessでもサイズが大きくならない
    execute if data storage asset:context this{Small:true} run tag @s add 8001.Small
...
# 攻撃位置表示
    ...
    data modify storage api: Argument.FieldOverride.Color set value 16737380
    ...
    execute if predicate api:global_vars/difficulty/min/3_blessless unless entity @s[tag=8001.Small] run data modify storage api: Argument.FieldOverride.Scale set value [10f, 10f, 0.05f]
    execute if predicate api:global_vars/difficulty/min/3_blessless if entity @s[tag=8001.Small] run data modify storage api: Argument.FieldOverride.Scale set value [7f, 7f, 0.05f]
