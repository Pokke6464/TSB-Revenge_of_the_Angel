#> asset:object/8001.eclael_thunder_red/tick/damage
#
# Objectのtick時の処理
#
# @within function asset:object/8001.eclael_thunder_red/tick/

# ヒット判定
    execute if predicate api:global_vars/difficulty/max/2_hard as @a[tag=!PlayerShouldInvulnerable,distance=..3.5] run tag @s add 8001.Hit
    execute if predicate api:global_vars/difficulty/min/3_blessless unless entity @s[tag=8001.Small] as @a[tag=!PlayerShouldInvulnerable,distance=..4.9] run tag @s add 8001.Hit
    execute if predicate api:global_vars/difficulty/min/3_blessless if entity @s[tag=8001.Small] as @a[tag=!PlayerShouldInvulnerable,distance=..3.5] run tag @s add 8001.Hit
...
# ダメージ
    ...
    execute as @a[tag=8001.Hit] run function api:damage/
...
# Effect(極光侵蝕・紅)を付与
    ...
    # Effectを付与
        ...
        execute unless data storage asset:context this{ShortDebuff:true} run data modify storage api: Argument.Duration set value 200
        execute if data storage asset:context this{ShortDebuff:true} run data modify storage api: Argument.Duration set value 100
        execute as @a[tag=8001.Hit] run function api:entity/mob/effect/give
        function api:entity/mob/effect/reset
    # リセット
        scoreboard players reset $Difficulty Temporary
        tag @a remove 8001.Hit
