...
# パターン決定
    execute store result score @s[scores={A5.Phase=1}] Temporary run random value 0..4
    execute if score @s Temporary matches 0..2 run tag @s add A5.HoloBlock.1
    execute if score @s Temporary matches 3 run tag @s add A5.HoloBlock.2
    execute if score @s Temporary matches 4 run tag @s add A5.HoloBlock.3
    scoreboard players reset @s Temporary

# 第2形態では必ず拡散型になる
    execute if score @s A5.Phase matches 2.. run tag @s add A5.HoloBlock.1
