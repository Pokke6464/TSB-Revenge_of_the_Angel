...
# パターン発動
    execute if entity @s[tag=A5.HoloBlock.1,scores={A5.Phase=..1}] if score @s General.Mob.Tick matches 15..25 run function asset:mob/0365.frestchika/tick/base_move/skill/holo_block/pattern/1
...
# 第2形態ではパターン1が少し増量+プレイヤーから少しずらした場所を起点にする(ずらさないとプレイヤーがよく埋まる)
    execute if entity @s[tag=A5.HoloBlock.1,scores={A5.Phase=2..}] if score @s General.Mob.Tick matches 15..25 at @p[tag=!PlayerShouldInvulnerable] positioned ~2 ~ ~2 run function asset:mob/0365.frestchika/tick/base_move/skill/holo_block/pattern/1
    execute if entity @s[tag=A5.HoloBlock.1,scores={A5.Phase=2..}] if score @s General.Mob.Tick matches 15..25 if predicate lib:random_pass_per/10 at @p[tag=!PlayerShouldInvulnerable] positioned ~-2 ~ ~-2 run function asset:mob/0365.frestchika/tick/base_move/skill/holo_block/pattern/1
