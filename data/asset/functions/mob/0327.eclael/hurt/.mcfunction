...
# 特殊怯み処理
    # 居眠り
        execute if data storage asset:context Hurt{IsDoT:false} if entity @s[tag=93.Skill.Former.Idle.Sleep,tag=!93.Temp.SleepDamage] run tag @s add 93.Temp.SleepDamage
    # 前半・居合斬り
        execute if data storage asset:context Hurt{IsDoT:false} if entity @s[tag=93.Temp.NotArmor,tag=93.Skill.Former.Iai] if score @s 93.DamageIntervalTimer matches ..0 if entity @a[tag=Attacker,distance=..4] run function asset:mob/0327.eclael/tick/app.skill_events/01_former_iai/6.start_damage_animation
