...
# 攻撃位置決定
    summon area_effect_cloud ^ ^ ^ {Duration:11,Tags:["93.Temp.AttackPosition"]}
    # 偏差
        execute if predicate api:global_vars/difficulty/min/3_blessless if entity @s[tag=93.Temp.Predict] run function asset:mob/0327.eclael/tick/app.skill_events/38_latter_circle_slash/attack_predict
# 地上に合わせる
