...
# 攻撃
    ...
    # 移動
        ...
        execute if score @s 93.AnimationTimer matches 7 at @p[tag=!PlayerShouldInvulnerable,distance=..50] facing entity @s feet rotated ~ 0 run function asset:mob/0327.eclael/tick/app.skill_events/36_latter_iai/set_move_position
...
# ハードでは折り返す
    ...
    # 強化後半戦ならスピードアップ
        execute if score @s 93.AnimationTimer matches 38 if entity @s[tag=93.Phase.Latter.Extra,tag=!93.LowHP] unless entity @a[tag=93.Skill.IaiMove.Hit,distance=..80] run function asset:mob/0327.eclael/tick/app.skill_events/36_latter_iai/turn
