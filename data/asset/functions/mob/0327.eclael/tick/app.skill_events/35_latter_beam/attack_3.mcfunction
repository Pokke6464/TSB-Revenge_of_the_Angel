...
# 弾召喚
    execute unless entity @s[tag=93.Skill.Beam.ReflectType] run function asset:mob/0327.eclael/tick/app.skill_events/35_latter_beam/attack_beam.m {ID:2190,Damage:"0",Angle:0.05f,SlowSpeed:"false"}
    execute if entity @s[tag=93.Skill.Beam.ReflectType] run return run function asset:mob/0327.eclael/tick/app.skill_events/35_latter_beam/attack_beam.m {ID:8002,Damage:"1",Angle:0.05f,SlowSpeed:"false"}

# ライン斬撃
    execute if predicate api:global_vars/difficulty/min/3_blessless run function asset:mob/0327.eclael/tick/app.skill_events/35_latter_beam/attack_line_slash.m {Rotation:60,PosX:0,PosY:0,PosZ:3,IsGreen:"false"}
    execute if predicate api:global_vars/difficulty/min/3_blessless run function asset:mob/0327.eclael/tick/app.skill_events/35_latter_beam/attack_line_slash.m {Rotation:120,PosX:0,PosY:0,PosZ:3,IsGreen:"true"}
    # summon area_effect_cloud ^ ^ ^ {Duration:1,Tags:["93.Temp.AttackRotation"]}
    # execute if predicate api:global_vars/difficulty/min/3_blessless as @e[type=area_effect_cloud,tag=93.Temp.AttackRotation,distance=..80] run tp @s ~ ~ ~ ~60 0
    ...
    # execute if predicate api:global_vars/difficulty/min/3_blessless as @e[type=area_effect_cloud,tag=93.Temp.AttackRotation,distance=..80] at @s run tp @s ~ ~ ~ ~60 0
    # execute if predicate api:global_vars/difficulty/min/3_blessless run data modify storage api: Argument.ID set value 2207
    # execute if predicate api:global_vars/difficulty/min/3_blessless store result storage api: Argument.FieldOverride.MobUUID int 1 run scoreboard players get @s MobUUID
