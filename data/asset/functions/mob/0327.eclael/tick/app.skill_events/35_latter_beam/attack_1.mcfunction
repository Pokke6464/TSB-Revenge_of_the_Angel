#> asset:mob/0327.eclael/tick/app.skill_events/35_latter_beam/attack_1
#
# アニメーションのイベントハンドラ
#
...
# 演出
    ...
    execute positioned ^ ^1.6 ^ rotated ~-90 ~-10 run function asset:mob/0327.eclael/tick/app.skill_events/35_latter_beam/particle_slash
    execute positioned ^ ^1.6 ^3 rotated ~-90 ~-10 run function asset:mob/0327.eclael/tick/app.skill_events/35_latter_beam/particle_slash
    execute positioned ^ ^1.6 ^6 rotated ~-90 ~-10 run function asset:mob/0327.eclael/tick/app.skill_events/35_latter_beam/particle_slash
...
# 強化後半戦の雷(ほぼ飾り)
    execute if entity @s[tag=93.Phase.Latter.Extra,tag=!93.LowHP] rotated ~25 ~ positioned ^ ^ ^9 run function asset:mob/0327.eclael/tick/app.general/16.thunderstorm_attack.m {Delay:0,Silent:"false"}
    execute if entity @s[tag=93.Phase.Latter.Extra,tag=!93.LowHP] rotated ~-25 ~ positioned ^ ^ ^9 run function asset:mob/0327.eclael/tick/app.general/16.thunderstorm_attack.m {Delay:0,Silent:"false"}

# 弾召喚
    execute unless entity @s[tag=93.Skill.Beam.ReflectType] run function asset:mob/0327.eclael/tick/app.skill_events/35_latter_beam/attack_beam.m {ID:2190,Damage:"0",Angle:3.31613f,SlowSpeed:"false"}
    execute if entity @s[tag=93.Skill.Beam.ReflectType] run return run function asset:mob/0327.eclael/tick/app.skill_events/35_latter_beam/attack_beam.m {ID:8002,Damage:"1",Angle:3.31613f,SlowSpeed:"false"}

# ライン斬撃
    execute if predicate api:global_vars/difficulty/2_hard run function asset:mob/0327.eclael/tick/app.skill_events/35_latter_beam/attack_line_slash.m {Rotation:0,PosX:0,PosY:0,PosZ:0,IsGreen:"false"}
    execute if predicate api:global_vars/difficulty/min/3_blessless run function asset:mob/0327.eclael/tick/app.skill_events/35_latter_beam/attack_line_slash.m {Rotation:0,PosX:2.75,PosY:0,PosZ:0,IsGreen:"false"}
    execute if predicate api:global_vars/difficulty/min/3_blessless run function asset:mob/0327.eclael/tick/app.skill_events/35_latter_beam/attack_line_slash.m {Rotation:0,PosX:-2.75,PosY:0,PosZ:0,IsGreen:"false"}
    # summon area_effect_cloud ^ ^ ^ {Duration:1,Tags:["93.Temp.AttackRotation"]}
    # execute if predicate api:global_vars/difficulty/min/3_blessless as @e[type=area_effect_cloud,tag=93.Temp.AttackRotation,distance=..80] run tp @s ~ ~ ~ ~ 0
    ...
    # execute if predicate api:global_vars/difficulty/min/3_blessless positioned ^ ^ ^ run function api:object/summon
