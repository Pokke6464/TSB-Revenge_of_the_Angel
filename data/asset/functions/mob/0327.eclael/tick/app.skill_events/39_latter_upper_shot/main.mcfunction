...
# 開始
    execute if score @s 93.AnimationTimer matches 1 run function asset:mob/0327.eclael/tick/app.skill_events/39_latter_upper_shot/animation_0.m {to_frame:1}
    # 中心点を挟んで反対側に移動
        execute if score @s 93.AnimationTimer matches 5 facing entity @p[tag=!PlayerShouldInvulnerable] feet rotated ~ 0 positioned as @e[type=marker,tag=93.Marker.SpawnPoint,distance=..80,sort=nearest,limit=1] run tp @s ^ ^-2.3 ^-18 ~ 0
    # プレイヤーの方を向く
...
# 曲射 / Blesslessの通常後半戦の場合のみダメージエリアの進むスピードが遅くなる
    execute if score @s 93.AnimationTimer matches 10 if predicate api:global_vars/difficulty/min/3_blessless unless entity @s[tag=93.Phase.Latter.Extra,tag=!93.LowHP] at @s run return run function asset:mob/0327.eclael/tick/app.skill_events/39_latter_upper_shot/attack_3
    execute if score @s 93.AnimationTimer matches 10 run function asset:mob/0327.eclael/tick/app.skill_events/39_latter_upper_shot/attack_0
    # 演出
...
# 強化後半戦なら曲射以降はスキップ
    execute if entity @s[tag=93.Phase.Latter.Extra,tag=!93.LowHP] if score @s 93.AnimationTimer matches 65 run function asset:mob/0327.eclael/tick/app.skill_events/39_latter_upper_shot/animation_0.m {to_frame:165}
    execute if entity @s[tag=93.Phase.Latter.Extra,tag=!93.LowHP] if score @s 93.AnimationTimer matches 65 run scoreboard players set @s 93.AnimationTimer 165

# 射撃1
    execute if score @s 93.AnimationTimer matches 105 at @s run function asset:mob/0327.eclael/tick/app.skill_events/39_latter_upper_shot/prediction_0
...
# 射撃2
    execute if score @s 93.AnimationTimer matches 125 at @s run function asset:mob/0327.eclael/tick/app.skill_events/39_latter_upper_shot/prediction_1
...
# 難易度がHard以上なら繰り返す
    execute unless entity @s[tag=93.Phase.Latter.Extra,tag=!93.LowHP] if score @s 93.AnimationTimer matches 168 if predicate api:global_vars/difficulty/min/2_hard run function asset:mob/0327.eclael/tick/app.skill_events/39_latter_upper_shot/turn
