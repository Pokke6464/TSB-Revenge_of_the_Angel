#> asset:mob/0327.eclael/tick/app.skill_events/39_latter_upper_shot/main_turn
#
# アニメーションのイベントハンドラ 曲射
#
...
# 開始
    execute if score @s 93.AnimationTimer matches 1 run function asset:mob/0327.eclael/tick/app.skill_events/39_latter_upper_shot/animation_2
    # 現在地から反対側に移動
        execute if score @s 93.AnimationTimer matches 5 facing entity @e[type=marker,tag=93.Marker.SpawnPoint,distance=..80,sort=nearest,limit=1] feet rotated ~ 0 positioned as @e[type=marker,tag=93.Marker.SpawnPoint,distance=..80,sort=nearest,limit=1] run tp @s ^ ^-2.3 ^18 ~180 0
    # プレイヤーの方を向く
...
# 攻撃
    ...
    execute if score @s 93.AnimationTimer matches 105 at @s run function asset:mob/0327.eclael/tick/app.skill_events/39_latter_upper_shot/prediction_2
    execute if score @s 93.AnimationTimer matches 120 at @s run function asset:mob/0327.eclael/tick/app.skill_events/39_latter_upper_shot/attack_4
...
# 終了
    execute if score @s 93.AnimationTimer matches 180 run function asset:mob/0327.eclael/tick/app.skill_events/39_latter_upper_shot/animation_1
    # 移動
        execute if score @s 93.AnimationTimer matches 175 positioned as @e[type=marker,tag=93.Marker.SpawnPoint,distance=..80,sort=nearest,limit=1] run tp @s ~ ~-2 ~ ~ 0
    # 演出
        execute if score @s 93.AnimationTimer matches 180 run playsound entity.wither.shoot hostile @a[distance=..30] ~ ~ ~ 0.5 1.8 0.5
        execute if score @s 93.AnimationTimer matches 180 run playsound entity.guardian.attack hostile @a ~ ~ ~ 2 1.8
        execute if score @s 93.AnimationTimer matches 180 run particle flash ~ ~3 ~ 0 0 0 0 1

# 終了
    execute if score @s 93.AnimationTimer matches 205.. run function asset:mob/0327.eclael/tick/app.skill_events/39_latter_upper_shot/end