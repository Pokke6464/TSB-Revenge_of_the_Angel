#> asset:mob/0327.eclael/tick/app.skill_events/33_latter_moveslash/attack_3
#
# アニメーションのイベントハンドラ
#
...
# MPを減少させる
    execute as @a[tag=93.Temp.AttackTarget] run function asset:mob/0327.eclael/tick/app.general/mp_reduce.m {Percent:1.5}
# リセット
...
# 演出
    ...
    execute rotated ~-60 ~-30 positioned ^ ^0 ^2 run function api:object/summon
...
# 周囲に雷を降らせる
    ...
    execute as @e[type=area_effect_cloud,tag=93.Temp.AttackPosition,distance=..80,sort=nearest,limit=1] at @s run function asset:mob/0327.eclael/tick/app.skill_events/33_latter_moveslash/attack_thunder.m {Distance:19}
    execute as @e[type=area_effect_cloud,tag=93.Temp.AttackPosition,distance=..80,sort=nearest,limit=1] at @s run tp @s ~ ~ ~ ~45 ~
    execute as @e[type=area_effect_cloud,tag=93.Temp.AttackPosition,distance=..80,sort=nearest,limit=1] at @s run function asset:mob/0327.eclael/tick/app.skill_events/33_latter_moveslash/attack_thunder.m {Distance:19}