...
# ガード解除
    execute if score @s 93.AnimationTimer matches 5 run function asset:mob/0327.eclael/tick/app.general/12.end_guard_prepare

# 後隙用の待機 / 本来なら60tickだが自然になるように少し調整している
# animated javaアニメーション再生 (長さ：20tick)
    execute if score @s 93.AnimationTimer matches 14 run function asset:mob/0327.eclael/tick/app.skill_events/25_latter_guard/3.4.play_idle_animation

# 終了
    execute if score @s 93.AnimationTimer matches 34.. run function asset:mob/0327.eclael/tick/app.skill_events/25_latter_guard/2.end
