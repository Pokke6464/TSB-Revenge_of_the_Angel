...
# animated javaアニメーション再生 (長さ：21tick)
    execute if score @s 93.AnimationTimer matches 116 run function asset:mob/0327.eclael/tick/app.skill_events/04_former_upper_shot/3_2.play_end_animation
# プレイヤーの方を向く
    execute if score @s 93.AnimationTimer matches 116..130 run tag @s add 93.Temp.Me
    execute if score @s 93.AnimationTimer matches 116..130 as @a[tag=!PlayerShouldInvulnerable,distance=..80,sort=nearest,limit=1] run function asset:mob/0327.eclael/tick/app.general/1.rotate
# 移動
    execute if score @s 93.AnimationTimer matches 116..125 at @s positioned ^ ^-0.2 ^-0.2 run function asset:mob/0327.eclael/tick/app.general/2.teleport
    execute if score @s 93.AnimationTimer matches 126..135 at @s positioned ^ ^-0.05 ^-0.05 run function asset:mob/0327.eclael/tick/app.general/2.teleport
# 演出
    execute if score @s 93.AnimationTimer matches 116 run playsound entity.phantom.flap hostile @a ~ ~ ~ 1 0.5

# 終了
    execute if score @s 93.AnimationTimer matches 137.. run function asset:mob/0327.eclael/tick/app.skill_events/04_former_upper_shot/2.end