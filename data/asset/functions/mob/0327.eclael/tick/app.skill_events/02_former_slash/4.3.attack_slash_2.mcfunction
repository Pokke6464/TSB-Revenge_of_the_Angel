...
# MPを減少させる
    execute positioned ^ ^ ^3 as @a[tag=!PlayerShouldInvulnerable,distance=..3.3] run function asset:mob/0327.eclael/tick/app.general/mp_reduce.m {Percent:3}
# リセット
...
# 弾召喚
    # 反射タイプなら反射弾になる
        execute unless entity @s[tag=93.Skill.Former.Slash.ReflectType] run data modify storage api: Argument.ID set value 2190
        execute unless entity @s[tag=93.Skill.Former.Slash.ReflectType] run data modify storage api: Argument.FieldOverride.Damage set from storage asset:context this.Damage.Beam0
        execute if entity @s[tag=93.Skill.Former.Slash.ReflectType] run data modify storage api: Argument.ID set value 8002
        execute if entity @s[tag=93.Skill.Former.Slash.ReflectType] run data modify storage api: Argument.FieldOverride.Damage set from storage asset:context this.Damage.Beam1
        execute if entity @s[tag=93.Skill.Former.Slash.ReflectType] run data modify storage api: Argument.FieldOverride.ExtendDuration set value true
