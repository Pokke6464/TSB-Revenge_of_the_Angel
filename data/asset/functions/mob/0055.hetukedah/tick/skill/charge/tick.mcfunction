...
# 後半戦では予備動作短縮
    execute if score @s[tag=1J.Phase.2] General.Mob.Tick matches 0 run scoreboard players set @s General.Mob.Tick 20
...
# Hard以上ならループする
    execute if predicate api:global_vars/difficulty/2_hard if score @s[scores={1J.LoopCount=..1}] General.Mob.Tick matches 60 run scoreboard players set @s General.Mob.Tick 25
