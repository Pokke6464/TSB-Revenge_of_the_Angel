...
# HurtTime:0sならダメージを与える
    execute as @e[type=zombie,tag=this,distance=..3,sort=nearest,limit=1] if entity @s[tag=!1J.Phase.2] run data modify storage api: Argument.Damage set value 7.0f
    execute as @e[type=zombie,tag=this,distance=..3,sort=nearest,limit=1] if entity @s[tag=1J.Phase.2] run data modify storage api: Argument.Damage set value 7.5f
