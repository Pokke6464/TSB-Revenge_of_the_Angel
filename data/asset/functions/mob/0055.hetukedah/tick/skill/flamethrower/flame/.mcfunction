...
# 再帰(ノーマル)
    execute if predicate api:global_vars/difficulty/1_normal if entity @s[distance=..12] positioned ^ ^ ^1 run function asset:mob/0055.hetukedah/tick/skill/flamethrower/flame/

# 再帰(ハード以上、長い)
    execute if predicate api:global_vars/difficulty/min/2_hard if entity @s[distance=..24] positioned ^ ^ ^1 run function asset:mob/0055.hetukedah/tick/skill/flamethrower/flame/
