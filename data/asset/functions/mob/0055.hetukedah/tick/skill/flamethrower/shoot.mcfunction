#> asset:mob/0055.hetukedah/tick/skill/flamethrower/shoot
#
#
#
# @within function asset:mob/0055.hetukedah/tick/skill/flamethrower/tick

#> インターバルのスコアホルダー
# @private
    #declare score_holder $SoundInterval

# 実行時間を移す
    scoreboard players operation $SoundInterval Temporary = @s General.Mob.Tick

# 数Tickごとにサウンド
    scoreboard players operation $SoundInterval Temporary %= $6 Const
    execute if score $SoundInterval Temporary matches 0 run playsound minecraft:entity.blaze.shoot hostile @a ~ ~ ~ 2 2
    scoreboard players reset $SoundInterval Temporary

# 火を放つ
    execute if entity @s[tag=!1J.ReverseTurn] anchored eyes positioned ^ ^-0.5 ^1 run function asset:mob/0055.hetukedah/tick/skill/flamethrower/flame/
    # 反転
        execute if entity @s[tag=1J.ReverseTurn] anchored eyes positioned ^ ^-0.5 ^1 run function asset:mob/0055.hetukedah/tick/skill/flamethrower/flame/reverse

# 手元のパーティクル
    execute anchored eyes positioned ^ ^-0.5 ^1 run particle smoke ~ ~ ~ 0.2 0.2 0.2 0.05 10

# 回転方向
    execute if entity @s[tag=1J.Turn.Left,tag=!1J.ReverseTurn] run tp @s ~ ~ ~ ~-2.5 ~
    execute if entity @s[tag=!1J.Turn.Left,tag=!1J.ReverseTurn] run tp @s ~ ~ ~ ~2.5 ~

# 逆回転+加速
    execute if entity @s[tag=1J.Turn.Left,tag=1J.ReverseTurn] run tp @s ~ ~ ~ ~8.5 ~
    execute if entity @s[tag=!1J.Turn.Left,tag=1J.ReverseTurn] run tp @s ~ ~ ~ ~-8.5 ~
